...
* **辅助函数**：
    ...
    * $\mathrm{PBKDF2}(\mathrm{HMAC}, \textit{input}, \textit{iters}, \textit{count})$：密钥派生函数，取前
      $\textit{count}$ 个字节。
* **存储原语**：
    * `DB.checkAccount`：对账户、身份属性或注册状态进行状态判断。
    * `DB.fetchAccount`：读取账户关联的信封、承诺或其他账户数据。
    ...
    * `DB.purgeVoteStart`：启动投票清理过程。
    * `DB.purgeVoteProcess`：按批次返回候选投票记录，并接收服务端对该批记录的清理结果。
    * `DB.purgeVoteFinish`：完成投票清理过程。
* **系统参数**：
    * $seed$：服务端持有的私密派生令牌。
    * $uuid$：用户唯一标识符。
    * $role$：凭证中的角色属性。
...
sequenceDiagram
    ...
    Note over S: 完成注册
    S->>DB: DB.checkAccount
    DB-->>S: DB.checkAccount.ack
...
6. **服务端完成注册**：
    * 服务端执行 `DB.checkAccount` 检查用户身份属性、账户归属信息及已有注册状态。
    * 注册阶段要求检查结果为 “不存在”，或为 “已匹配” 且与本次提交的身份属性一致；若检查结果为 “不匹配” 则注册失败。
    * 服务端确认提交的覆盖凭证与当前身份属性一致，并完成账户绑定。
    * 服务端执行 `DB.storeAccount` 存储 $(S, \epsilon)$ 及必要的账户绑定状态。
...
1. **客户端发起登录请求**：
    ...
    * 服务端执行 `DB.checkAccount` 检查用户 $uuid$ 是否存在。
2. **服务端生成登录响应**：
    * 若检查结果为 “已匹配”，则服务端执行 `DB.fetchAccount` 读取 $\epsilon$，并结合 $v$ 计算响应后返回
...
1. **客户端发起恢复请求**：
    ...
    * 服务端执行 `DB.checkAccount` 检查用户 $uuid$ 是否存在。
2. **服务端返回恢复材料**：
    * 服务端根据账户检查结果返回对应恢复材料后返回 `ServerPRFPresent`，和用户密码登录流程一致。
3. **客户端恢复私钥**：
    * 用户输入助记词 $mnem$。
...
2. **服务端确认身份属性并执行盲签名**：
    * 服务端先执行 `DB.checkAccount` 检查用户身份属性。
    * 签名授权阶段要求该用户身份属性的检查结果为 “已匹配”，否则签发失败。
...
sequenceDiagram
    ...
    Note over S: 追踪并清理投票
    S->>DB: DB.purgeVoteStart
    DB-->>S: DB.purgeVoteStart.ack
    S->>DB: DB.purgeVoteProcess
    DB-->>S: DB.purgeVoteProcess.ack
    S->>DB: DB.purgeVoteFinish
    DB-->>S: DB.purgeVoteFinish.ack
```
...
2. **服务端验证撤销归属**：
    * 服务端先执行 `DB.checkAccount` 检查用户身份属性。
    * 撤销授权阶段要求该用户身份属性的检查结果为 “已匹配”，否则撤销失败。
    * 服务端执行 `DB.fetchAccount` 读取与当前账户关联的承诺数据，以进行后续配对校验。
    * 服务端核验 $e(S, \tilde{G}_{neg}) \cdot e(G, \tilde{C}) = 1_T$。
3. **服务端追踪并清理投票**：
    * 服务端执行 `DB.storeRevocation` 将 $\tilde{C}$ 放入撤销列表。
    * 服务端执行 `DB.purgeVoteStart` 启动投票清理过程。
    * 服务端对 `DB.purgeVoteProcess` 重复执行，每次获取并检索其中满足
      $e(ID, \tilde{G}) \stackrel{?}{=} e(P_{work}, \tilde{C})$ 的所有 $ID$ 并回传。
    * 服务端执行 `DB.purgeVoteFinish` 完成对应 $info$ 与投票记录的清理。
...
* **Storage Primitives**:
    * `DB.checkAccount`: Checks account, identity-attribute, or registration state.
    * `DB.fetchAccount`: Reads account-associated envelopes, commitments, or other account data.
    ...
    * `DB.purgeVoteStart`: Starts the vote cleanup procedure.
    * `DB.purgeVoteProcess`: Returns a batch of candidate vote records and accepts the cleanup result for that batch.
    * `DB.purgeVoteFinish`: Finishes the vote cleanup procedure.
* **System Parameters**:
...
6. **Server Completes Registration**:
    * Server executes `DB.checkAccount` to check user identity attributes, account ownership, and existing registration
      state.
    * During registration, the check result must be “not found”, or “matched” with identity attributes consistent with
      the current submission; if the result is “mismatched”, registration fails.
    * Server confirms that the submitted override credential is consistent with the current identity attributes and
      completes account binding.
    * Server executes `DB.storeAccount` to store $(S, \epsilon)$ and the necessary bound account state.
...
1. **Client Starts the Login Request**:
    ...
    * Server executes `DB.checkAccount` to check whether user $uuid$ exists.
2. **Server Generates the Login Response**:
    * If the check result is “matched”, server executes `DB.fetchAccount` to read the stored $\epsilon$, then combines
      it with the real parameter $v$ to compute the response and returns `ServerPRFPresent` $(N, \epsilon)$.
    * If the check result is “not found”, server
...
2. **Server Returns the Recovery Material**:
    * Server returns the corresponding recovery material according to the account-check result and then returns
...
2. **Server Confirms Identity Attributes and Performs Blind Signing**:
    * Server first executes `DB.checkAccount` to check user identity attributes.
    * During signature authorization, the identity-attribute check result must be “matched”; otherwise issuance fails.
    * Server confirms user identity $S$ and assigns $role$.
...
2. **Server Verifies Revocation Ownership**:
    * Server first executes `DB.checkAccount` to check user identity attributes.
    * During revocation authorization, the identity-attribute check result must be “matched”; otherwise revocation
      fails.
    * Server executes `DB.fetchAccount` to read the account-associated commitment data for the subsequent pairing check.
    * Server verifies $e(S, \tilde{G}_{neg}) \cdot e(G, \tilde{C}) = 1_T$.
3. **Server Traces and Cleans Up Votes**:
    * Server executes `DB.storeRevocation` to place $\tilde{C}$ into the revocation list.
    * Server executes `DB.purgeVoteStart` to start the vote cleanup procedure.
    * Server repeatedly executes `DB.purgeVoteProcess`; on each iteration it obtains a batch of candidates and
      identifies all $ID$ satisfying $e(ID, \tilde{G}) \stackrel{?}{=} e(P_{work}, \tilde{C})$, then sends them back.
    * Server executes `DB.purgeVoteFinish` to finish cleanup of the corresponding $info$ and vote records.
