...
* **辅助函数**：
    * $\mathrm{PBKDF2}(\textit{input}, \textit{iters})$：基于密码的密钥派生函数（针对 $pass$ 与 $mnem$ 进行计算时使用不同盐值）。
    * $\mathrm{AES}$ / $\mathrm{AES}^{-1}$：采用 256 位密钥的 AES-GCM 对称加解密算法。
* **服务端密钥对**：
...
1. **初始请求与信封提交**：
    ...
    * 客户端生成随机 `ClientSecretKey` $s \in \mathbb{Z}_p$，计算 $P_{pass} = H_p(pass)$。
    * 选取随机数 $r \in \mathbb{Z}_p$，并记录 `ClientPRFSession` $\rho = (r, P_{pass}, s)$。
    * 计算并发送 `ClientPRFRequest` $M = r \cdot P_{pass}$。
    * 服务端检索发现该用户未注册，返回 `ServerPRFAbsent`，包含评估值 $N = v \cdot M$。
    * 客户端通过 $\rho$ 中的 $r$ 与 $s$ 计算信封元组 `ClientPRFOverride` $\eta = (E_{pass}, E_{mnem})$ 并提交：
        * $E_{pass} = \mathrm{AES}(\mathrm{PBKDF2}(r^{-1} \cdot N, 2048), s)$
        * $E_{mnem} = \mathrm{AES}(\mathrm{PBKDF2}(mnem, 2048), s)$

2. **查询与响应阶段**：

    * **口令模式**：客户端初始化 $\rho = (r, P_{pass}, 0)$，发送 `ClientPRFRequest` $M = r \cdot P_{pass}$。
    * **恢复模式**：用户输入 $mnem$ 与欲重置的新口令 $pass$。客户端初始化 $\rho = (r, P_{pass}, 0)$，发送对应新口令的
      `ClientPRFRequest` $M$。
    * 服务端返回 `ServerPRFPresent`，包含 $N = v \cdot M$ 以及对应模式的信封（$E_{pass}$ 或 $E_{mnem}$）。

3. **还原秘密种子并更新会话**：
    * **口令模式**：校验 $e(N, \tilde{G}_{neg}) \cdot e(M, \tilde{V}) = 1_T$。成功后还原 $s$：
      $s = \mathrm{AES}^{-1}(H_s(pass, r^{-1} \cdot N), E_{pass})$。随后将 $s$ 填入 $\rho$。
    * **恢复模式**：
        * 客户端通过助记词还原 $s = \mathrm{AES}^{-1}(\mathrm{PBKDF2}(mnem, 2048), E_{mnem})$。随后将 $s$ 填入 $\rho$。
        * 校验 $e(N, \tilde{G}_{neg}) \cdot e(M, \tilde{V}) = 1_T$。成功后根据新口令生成的 $N$
          计算 $E_{pass} = \mathrm{AES}(H_s(pass, r^{-1} \cdot N), s)$，通过 `ClientPRFOverride` 更新。
...
| `ClientPRFOverride`  | $\eta = (E_{pass}, E_{mnem})$                       | $120$                                           |
...
* **Auxiliary Functions**:
    * $\mathrm{PBKDF2}(\textit{input}, \textit{iters})$: A password-based key derivation function (using distinct salts
      for processing $pass$ and $mnem$).
    * $\mathrm{AES}$ / $\mathrm{AES}^{-1}$: Symmetric encryption and decryption utilizing the AES-GCM algorithm with a
      256-bit key..
* **Server Key Pair**:
...
1. **Initial Request and Envelope Submission**:
    * Client generates a random `ClientSecretKey` $s \in \mathbb{Z}_p$ and computes $P_{pass} = H_p(pass)$.
    * Client picks a random number $r \in \mathbb{Z}_p$ and records the `ClientPRFSession` $\rho = (r, P_{pass}, s)$.
    * Client calculates and sends `ClientPRFRequest` $M = r \cdot P_{pass}$.
    * Server determines that the user is not registered and returns `ServerPRFAbsent` containing the
      evaluation $N = v \cdot M$.
    * Client calculates the envelope tuple `ClientPRFOverride` $\eta = (E_{pass}, E_{mnem})$ using $r$ and $s$
      from $\rho$ and submits it:
        * $E_{pass} = \mathrm{AES}(\mathrm{PBKDF2}(r^{-1} \cdot N, 2048), s)$
        * $E_{mnem} = \mathrm{AES}(\mathrm{PBKDF2}(mnem, 2048), s)$

2. **Query and Response**:
    * **Password Mode**: Client initializes $\rho = (r, P_{pass}, 0)$, sends `ClientPRFRequest` $M = r \cdot P_{pass}$.
    * **Recovery Mode**: User inputs $mnem$ and a reset $pass$. Client initializes $\rho = (r, P_{pass}, 0)$ with the
      new password's point and sends $M$.
    * Server returns `ServerPRFPresent`, including $N$ and the respective envelope ($E_{pass}$ or $E_{mnem}$).

3. **Secret Seed Recovery and Session Update**:
    * **Password Mode**: Verify $e(N, \tilde{G}_{neg}) \cdot e(M, \tilde{V}) = 1_T$.
      Recover $s = \mathrm{AES}^{-1}(H_s(pass, r^{-1} \cdot N), E_{pass})$ and update $\rho$ with $s$.
    * **Recovery Mode**:
        * Recover $s = \mathrm{AES}^{-1}(\mathrm{PBKDF2}(mnem, 2048), E_{mnem})$ and update $\rho$ with $s$.
        * Verify $e(N, \tilde{G}_{neg}) \cdot e(M, \tilde{V}) = 1_T$. Calculate
          new $E_{pass} = \mathrm{AES}(H_s(pass, r^{-1} \cdot N), s)$ and update via `ClientPRFOverride`.
